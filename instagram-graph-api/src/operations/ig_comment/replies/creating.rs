...
use crate::{
    objects::{ig_comment::IgCommentAsReply, ResponseBodyErrJson},
    operations::{
...
//
//
pub type CreatingResponseBodyOkJson = IgCommentAsReply;
...
mod tests {
    ...
    fn test_de_response_body_ok_json() {
        //
        let content =
            include_str!("../../../../tests/response_body_json_files/v14.0/ig_comment_17897793971532270__replies__creating.json");
    ...
    fn test_endpoint_render_request() {
        ...
        assert_eq!(
            ...
            "/v15.0/1/replies?fields=from,hidden,id,like_count,parent_id,text,timestamp,username&message=foo&access_token=ACCESS_TOKEN"
        );
    }
}
